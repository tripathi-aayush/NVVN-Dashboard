...
# TRAIN FUNCTION
# ----------------------------------
def train_state_3hr_model(state: str):
    logger = get_logger(f"TRAIN-3H-{state}")
    ...
    # LAGS FOR 3 HOUR
    # -----------------------------
    for i in range(1, 4): # Last 3 periods = 9 hours
        df[f"y_lag_{i}"] = df["y"].shift(i)

    df["y_lag_24h"] = df["y"].shift(8) # 8 * 3H = 24H
    df["y_lag_7d"] = df["y"].shift(56) # 56 * 3H = 168H
    ...
    print("TRAIN ROWS 3H:", len(df), "COLS:", df.shape[1])
